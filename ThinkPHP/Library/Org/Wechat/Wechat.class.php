...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 */
	public function checkAuth($appid = '', $appsecret = '', $token = '', $useCache = false)
	{
        if (!$appid || !$appsecret) {
			$appid = $this->appid;
		 ...
		 * 2. 重新获取
		 */
		$key = 'WKZ::wechat::newaccesstoken';
		if ($useCache) {
			$cachedAT = \MyMemcache::getRecord($key);
			if ($cachedAT) {
				if (empty($this->jsapi_ticket)) {
					$this->getJsTicket();
				}
				$this->access_token = $cachedAT;
				return $this->access_token;
			}
		}
...
//			\MyMemcache::setRecord($key, $this->access_token, $expire);

			// 如果重新获取了accesstoken就重新获取下jsticket
			$this->getJSTicket('', '', false);
...
//		\MyMemcache::delRecord($key);
	 ...
	 */
	public function getJsTicket($appid = '', $jsapi_ticket = '', $useCache = false)
	{
//		if (!$this->access_token && !$this->checkAuth()) return false;
		...
		$key = 'WKZ::wechat::jsticket';
		if ($useCache) {
			$cachedTicket = \MyMemcache::getRecord($key);
			if ($cachedTicket) {
				$this->jsapi_ticket = $cachedTicket;
				return $this->jsapi_ticket;
			}
		}
		...
		if ($result) {
			...
			$expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
//			\MyMemcache::setRecord($key, $this->jsapi_ticket, $expire);
			return $this->jsapi_ticket;
		}
		return false;
	}
    ...
    public function getPublicSignPackage($url) {
        if (!$this->access_token && !$this->checkAuth()) return false;
