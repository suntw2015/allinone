<?php
	...
	class QiniuStorage {
		...
		public function setBuckets($buckets){
			if(strpos(array_values($this->buckets),$buckets)===false){
				return false;
			}
		...
		public function move($from_bucket, $from_key, $to_bucket, $to_key){
			$from = self::Qiniu_Encode("{$from_bucket}:{$from_key}");
			$to   = self::Qiniu_Encode("{$to_bucket}:{$to_key}");
			$url = "/move/".$from."/".$to;
			$accessToken = $this->accessToken($url);
			$response = $this->request($url, 'POST', array('Authorization'=>"QBox $accessToken"));
			return $response;
		...
		public function changeMime($bucket, $key, $mime){
			$resource = self::Qiniu_Encode("{$bucket}:{$key}");
			$encode_mime = self::Qiniu_Encode($mime);
			$url = '/chgm/'.$resource.'/mime/'.$encode_mime;
			$accessToken = $this->accessToken($url);
			$response = $this->request($url, 'POST', array('Authorization'=>"QBox $accessToken"));
			return $response;
		}
